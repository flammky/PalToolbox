...
import dev.dexsr.gmod.palworld.trainer.platform.content.filepicker.win32.Comdlg32Kt.CommDlgExtendedError
import dev.dexsr.gmod.palworld.trainer.platform.content.filepicker.win32.Comdlg32Kt.GetOpenFileNameW
import dev.dexsr.gmod.palworld.trainer.platform.content.filepicker.win32.Comdlg32Kt.GetSaveFileNameW
...
class WindowsFileChooser {
    ...
    var currentDirectory: File? = null
        protected set
    protected var _filters: ArrayList<Array<String>>

    protected var _defaultFilename: String = ""
    ...
    fun addFilter(name: String, vararg filter: String) {
        ...
        _filters.add(parts.toTypedArray<String>())
    }
    ...
    fun showDialog(parent: Window?, open: Boolean): Boolean {
        val params = Comdlg32Kt.OpenFileName()
        params.Flags =  // use explorer-style interface
            (Comdlg32Kt.OFN_EXPLORER // the dialog changes the current directory when browsing,
                    // this flag causes the original value to be restored after the
                    // dialog returns
                    or Comdlg32Kt.OFN_NOCHANGEDIR // disable "open as read-only" feature
                    or Comdlg32Kt.OFN_HIDEREADONLY // enable resizing of the dialog
                    or Comdlg32Kt.OFN_ENABLESIZING)
        ...
        if (open and _defaultFilename.isNotEmpty()) {
            params.lpstrFile!!.setWideString(0, _defaultFilename)
        } else {
            params.lpstrFile!!.clear(bufferSize.toLong())
        }
        if (!_dialogTitle.isEmpty()) {
            params.lpstrTitle = WString(_dialogTitle)
        }
